    ...
    <div className="w-screen">
      {jobs.map((job) => (
        ...
        >
          ...
          <Link href={`/jobs/${job.id}`} className="absolute bottom-2 right-2 font-semibold text-xs text-blue-700 p-2 rounded-md border border-gray-300 bg 
          bg-gray-100 hover:bg-gray-200 cursor-pointer transition-colors duration-200">VIEW JOB</Link>
        </Card>
      ))}
    </div>
  );
